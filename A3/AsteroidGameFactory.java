...
public void setAsteroid(Asteroid ast){
	...
	@Override
	public  BoardComponent MakeShield(){
	...
	public Command MakeCommand(String commandLine)
	{
		...
		{
			commandToMake = commandLine.split(" ")[0];// read only command string
			commandLineArgs = commandLine.substring(commandToMake.length() + 1);// separate command arguments from command string
			args = commandLineArgs.split(" ");// store command arguments
		}
		else
		{
...
//bhanu start
				int x = Integer.parseInt(args[0]);
				int y = Integer.parseInt(args[1]);
				BoardComponent square = GameBoard.Instance().GetBoard().get(y).get(x);// use/get a particular Square from the GRID/ BOARD  i.e. BOARD[0][0] or BOARD[0][1] e.t.c. to place the building

				Command cmd =new SpawnBuildingCommand(square, args);// create new building
				GameBoard.Instance().IncrementBuildingCount();// after creating the building increment the count
				return cmd;
...
//bhanu end
			...
			case "SPAWN_SHIELD":
			{
				...
				int x = Integer.parseInt(args[0]);
				int y = Integer.parseInt(args[1]);
				BoardComponent shield = GameBoard.Instance().GetBoard().get(y).get(x);// use/get a particular Square from the  board
				Command cmd =new SpawnShieldCommand(shield, args);// create new building
				return cmd;
